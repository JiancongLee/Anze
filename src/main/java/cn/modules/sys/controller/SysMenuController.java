...
import cn.modules.sys.service.SysMenuService;
...
public class SysMenuController extends AbstractController {
	...
	/*	EntityWrapper<SysMenuEntity> sysMenuEntityEntityWrapper = new EntityWrapper<>();
		sysMenuEntityEntityWrapper.eq("PARENT_ID","0");
		List<SysMenuEntity> menuList = sysMenuService.selectList(sysMenuEntityEntityWrapper);*/
		List<SysMenuEntity> menuList = Lists.newArrayList();
		if(getUserId() == Constant.SUPER_ADMIN){
			EntityWrapper<SysMenuEntity> sysMenuEntityEntityWrapper = new EntityWrapper<>();
			sysMenuEntityEntityWrapper.eq("PARENT_ID","0");
			menuList = sysMenuService.selectList(sysMenuEntityEntityWrapper);
		} else {
	...
	private void verifyForm(SysMenuEntity menu){
		if(StringUtils.isBlank(menu.getName())){
			throw new DefaultException("菜单名称不能为空");
		}
		
		if(menu.getParentId() == null){
			throw new DefaultException("上级菜单不能为空");
		}
